...
def homomorphic_decryption(x, input_power=3, weight_power=3):
    private_key = 73939133
    return (x % private_key) / 10 ** (input_power + weight_power)
    
def power_adjustment(x, weight_power=3):
    x = x * 10 ** weight_power
    x = K.cast(x,"int32")
    return K.cast(x,"float32")

class Dense_HD(Layer):
    """Just your regular densely-connected NN layer.
    ...
    def __init__(self, units,
        ...
        if 'input_shape' not in kwargs and 'input_dim' in kwargs:
            kwargs['input_shape'] = (kwargs.pop('input_dim'),)
        super(Dense_HD, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.use_bias = use_bias
        self.kernel_initializer = initializers.get(kernel_initializer)
    ...
    def call(self, inputs):
        output = K.dot(inputs, power_adjustment(self.kernel, self.weight_power))
        if self.use_bias:
            output = K.bias_add(output, power_adjustment(self.bias, self.weight_power), data_format='channels_last')
        if self.activation is not None:
            output = self.activation(homomorphic_decryption(output, self.input_power))
        return output
